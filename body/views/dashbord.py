...
class DashboardStatsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        """GET request for general dashboard stats"""
        try:
...
class UserDashboardStatsViewSet(viewsets.ViewSet):
    permission_classes = [permissions.IsAuthenticated]

    def list(self, request):
        """GET request for user-specific dashboard stats"""
        try:
            user = request.user
            ...
            user_participant_counts = GiftParticipant.objects.filter(gift__in=user_completed_gifts).values(
                'gift').annotate(
                count=Count('id', distinct=True)
            )
